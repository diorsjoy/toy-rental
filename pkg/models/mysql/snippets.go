...
// This will return the 10 most recently created snippets by ID.
...
func (m *SnippetModel) Students() ([]*models.Snippet, error) {
	stmt := `SELECT id, title, content FROM students
ORDER BY id ASC`
...
func (m *SnippetModel) InsertDepartments(dep_name, staff_quantity string) (int, error) {
...
func (m *SnippetModel) ShowDepartments() ([]*models.Snippet, error) {
...
ORDER BY id ASC`
	...
	for rows.Next() {
		s := &models.Snippet{}
		err = rows.Scan(&s.ID, &s.Dep_name, &s.Staff_quantity)
		if err != nil {
			return nil, err
		}
		snippets = append(snippets, s)
...
func (m *SnippetModel) GetDepartment(id int) (*models.Snippet, error) {
	stmt := `SELECT id, dep_name, staff_quantity FROM departments
WHERE id = ?`
	row := m.DB.QueryRow(stmt, id)
	s := &models.Snippet{}
	err := row.Scan(&s.ID, &s.Dep_name, &s.Staff_quantity)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, models.ErrNoRecord
		} else {
...
func (m *SnippetModel) GetToys() ([]*models.Toy, error) {
...
func (m *SnippetModel) GetToy(id int) (*models.Toy, error) {
	stmt := `SELECT id, name, description, tokens FROM toys
WHERE id = ?`
...
func (m *SnippetModel) InsertToy(name, desc, tokens string) (int, error) {
